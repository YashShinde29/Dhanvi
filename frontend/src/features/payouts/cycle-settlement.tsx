...
export function CycleSettlement({ cycleId, groupId, scope }: { cycleId: string; groupId: string; scope: GroupScope }) {
  ...
  return <Card><CardHeader title="Payout settlement" subtitle="Selection and money settlement are tracked separately." /><CardBody className="stack">
    ...
    <div className="row">{admin && <Button loading={busy} onClick={prepare}>Prepare cycle settlement</Button>}<LinkButton variant="secondary" href={admin ? `/admin/payouts?groupId=${groupId}&cycleId=${cycleId}` : organizer ? `/organizer/groups/${groupId}/payouts` : "/payouts"}>View payout status</LinkButton></div>
  </CardBody></Card>;
}
