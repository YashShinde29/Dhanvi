import { apiClient, enumParam } from "./api-client";
...
export const paymentService = {
  ...
  list: (admin: boolean, page: number, status?: string) => apiClient<PaymentPage>(`${admin ? "admin/" : ""}payments?page=${page}&pageSize=20${status ? `&status=${enumParam(status)}` : ""}`),
