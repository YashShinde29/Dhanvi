...
/**
 * Traps focus inside an open dialog. Runs only when the dialog opens/closes — never on ordinary re-renders.
 * `onClose` is read through a ref: parents commonly pass a new closure every render (e.g. `() => setOpen(false)`),
 * and re-running this effect per keystroke used to move focus away from the field being typed in.
 */
function useFocusTrap(open: boolean, ref: React.RefObject<HTMLDivElement | null>, onClose: () => void) {
  ...
  useEffect(() => {
    ...
    // React applies `autoFocus` before effects run; if focus is already inside the dialog, keep it there.
    if (!node?.contains(document.activeElement)) (node?.querySelector<HTMLElement>("[data-autofocus]") ?? focusable()[0] ?? node)?.focus();
