...
export function ChipGroup({ options, value, onChange, label, scroll }: { options: ChipOption[]; value: string; onChange: (value: string) => void; label: string; /** Phones: one scrolling row instead of wrapping (for 5+ chips). */ scroll?: boolean }) {
  return (
    <div className={`chip-row${scroll ? " chip-row--scroll" : ""}`} role="group" aria-label={label}>
