...
const journalColumns: Column<JournalSummary>[] = [
  ...
  { key: "group", header: "Group", render: j => j.groupId ? <Link className="link" title={j.groupId} href={`/ledger/groups/${j.groupId}`}>{short(j.groupId)}</Link> : "Platform" },
...
function JournalDetails() {
  ...
  return <div className="stack stack--lg"><Breadcrumbs items={[{ label: "Ledger", href: "/ledger" }, { label: j.journalNumber }]} /><PageHeader title={j.journalNumber} description={j.description} />
    ...
    {d.reversesJournalEntryId && <p>Reverses <Link className="link" href={`/ledger/journals/${d.reversesJournalEntryId}`}>original journal</Link>: {d.reversalReason}</p>}
    {d.reversedByJournalEntryId && <p>Corrected by <Link className="link" href={`/ledger/journals/${d.reversedByJournalEntryId}`}>reversal journal</Link>.</p>}
    </CardBody></Card>
