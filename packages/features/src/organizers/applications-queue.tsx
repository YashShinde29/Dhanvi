...
import type { OrganizerApplicationSummary } from "@dhanvi/types";
import { useAsyncData, useDebounced, friendlyError, formatDate, formatDateTime, ORGANIZER_APPLICATION_STATUSES, presentStatus } from "@dhanvi/utils";
import { PageHeader, Avatar, StatusBadge, Button, ErrorState, DataTable, Pagination, type Column, DescriptionList, Drawer, ChipGroup, SearchInput, useToast, useConfirm } from "@dhanvi/ui";
...
export function OrganizerApplicationsQueue() {
  ...
  async function reject(item: OrganizerApplicationSummary) {
    ...
    if (!result.confirmed) return;
    setBusyId(item.id);
    try { await organizerService.reject(item.id, result.reason ?? ""); toast.success("Application rejected", "The applicant sees your reason and may apply again."); setSelected(null); await reload(); }
