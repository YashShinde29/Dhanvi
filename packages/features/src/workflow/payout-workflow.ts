...
export function payoutSteps(p: Payout): WorkflowStep[] {
  ...
  return [
    { id: "account", label: "Bank account", state: s(0, "attention"), hint: p.maskedAccountNumber ? `Ending ${p.maskedAccountNumber.slice(-4)}` : r === 0 ? "Not added" : undefined },
...
export function payoutSummary(p: Payout, viewer: "member" | "organizer" | "admin", account?: PayoutAccount | null): WorkflowSummary {
...
export function payoutChecklist(p: Payout): { label: string; done: boolean; hint?: string }[] {
  const r = ["PENDING_BENEFICIARY", "APPROVAL_REQUIRED", "APPROVED", "PROCESSING", "PROVIDER_PENDING", "SUCCEEDED", "FAILED", "RECONCILIATION_REQUIRED"].indexOf(p.status);
  return [
    { label: "Funding verified", done: true, hint: "Prepared from a funded selection" },
    { label: "Beneficiary account", done: !!p.maskedAccountNumber || p.payoutType === "PLATFORM_FEE_SETTLEMENT", hint: p.maskedAccountNumber ? `Ending ${p.maskedAccountNumber.slice(-4)}` : "Recipient has not added a payout bank account" },
...
export function memberPayoutActions(payouts: Payout[], account: PayoutAccount | null | undefined): { actions: WorkflowAction[]; waiting: WorkflowAction[] } {
  ...
  for (const p of payouts.filter((x) => ACTIVE.includes(x.status))) {
...
export function payoutPriority(summary: Record<string, number> | undefined) {
  const n = (k: string) => summary?.[k] ?? 0;
  return [
    { id: "APPROVAL_REQUIRED", label: "Action required", count: n("APPROVAL_REQUIRED") + n("APPROVED") + n("FAILED"), tone: "warning" as const, hint: `${n("APPROVAL_REQUIRED")} to approve · ${n("APPROVED")} to execute · ${n("FAILED")} failed` },
    { id: "RECONCILIATION_REQUIRED", label: "Reconciliation", count: n("RECONCILIATION_REQUIRED"), tone: "danger" as const, hint: "Provider mismatch" },
    { id: "PENDING_BENEFICIARY", label: "Waiting on member", count: n("PENDING_BENEFICIARY"), tone: "info" as const, hint: "Bank account missing" },
    { id: "PROCESSING", label: "Waiting for provider", count: n("PROCESSING") + n("PROVIDER_PENDING"), tone: "info" as const, hint: "Transfer in progress" },
    { id: "SUCCEEDED", label: "Completed", count: n("SUCCEEDED"), tone: "success" as const, hint: "Settled" },
  ];
}
