...
export function auctionSummary(a: Auction, group: Group, viewer: "member" | "organizer" | "admin", canManage: boolean): WorkflowSummary {
  ...
  switch (a.status) {
    case "SCHEDULED": return { stage: g.stage, status: canManage && a.canOpen ? "attention" : "waiting", headline: `Starts ${when(a.startsAt)} · closes ${when(a.endsAt)}.`, detail: viewer === "member" ? "No action is required yet." : undefined, steps, responsibleRole: "ORGANIZER", waitingFor: canManage && a.canOpen ? undefined : "the organizer to open the auction", next: "Eligible members bid a discount; the highest valid discount wins.", action: canManage && a.canOpen ? { title: "Open the auction", status: "attention", responsibleRole: "ORGANIZER", actionLabel: "Open auction", onAction: undefined } : undefined };
