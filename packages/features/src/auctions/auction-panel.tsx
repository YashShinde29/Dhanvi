...
import { auctionService, type GroupScope } from "@dhanvi/api-client";
import type { Auction, AuctionBid, Group, MonthlyCycle } from "@dhanvi/types";
import { Badge, StatusBadge, Button, Callout, ErrorState, Card, CardBody, CardHeader, DataTable, type Column, Fact, KeyValueRows, FormField, MoneyInput, Icons, SkeletonText, useToast, useConfirm } from "@dhanvi/ui";
import { friendlyError, formatDateTime, formatMoney, formatSignedMoney, humanize } from "@dhanvi/utils";
import { AuctionResultCard } from "./auction-result";
import { WorkflowStatusCard, auctionSummary, bidUnavailableReason, UnavailableAction } from "../workflow";
...
export function AuctionPanel({ group, cycle, scope }: { group: Group; cycle: MonthlyCycle; scope: GroupScope }) {
  ...
  async function manage(action: "open" | "close") {
    ...
    try { setData(await auctionService.manage(scope, group.id, cycle.id, action)); toast.success(action === "open" ? "Auction opened ✓" : "Auction closed ✓", action === "open" ? "Next: eligible members place discount bids until it closes." : "Next: payout preparation for the winner."); }
  ...
  async function bid(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Discount bid submitted ✓", `Bid #${accepted.sequenceNumber}: ${formatMoney(accepted.discountAmount)} discount. Next: the highest valid discount wins when the auction closes.`);
      setDiscount("");
  ...
  const summary = auctionSummary(data, group, viewer, canManageScope && data.canManage);
  if (summary.action && data.canOpen) summary.action = { ...summary.action, onAction: () => manage("open") };
  return (
    <div className="stack stack--lg">
      <WorkflowStatusCard summary={summary} viewer={viewer} title={`Cycle ${cycle.cycleNumber} auction`} stepperLabel="Auction steps" />
      <Card>
        <CardHeader
        ...
        <CardBody className="stack stack--lg">
          ...
          {canManageScope && data.canManage && data.status === "SCHEDULED" && !data.canOpen && <UnavailableAction id={`auction-open-${cycle.id}`} label="Open auction" reason={cycle.status !== "READY_FOR_SELECTION" ? `${cycle.expectedMemberCount - (cycle.collectionMode === "RAZORPAY" ? cycle.financiallySettledMemberCount : cycle.fullyRecordedMemberCount)} member contribution(s) are still outstanding for this cycle.` : `The auction window opens ${when(data.startsAt)}; the server enforces the scheduled time.`} />}
          {canManageScope && data.canManage && data.status === "OPEN" && !data.canClose && <UnavailableAction id={`auction-close-${cycle.id}`} label="Close and finalize auction" reason={`The auction can be closed after its scheduled end ${when(data.endsAt)}.`} />}
      ...
      ) : data.bidUnavailableReason ? <Callout variant="neutral" title="You cannot bid in this cycle">Reason: {bidUnavailableReason(data.bidUnavailableReason)}</Callout> : null}
