...
import type { Contribution, ContributionEntry } from "@dhanvi/types";
import { useAsyncData, friendlyError, formatDate, formatDateTime, formatMoney, formatNumber, statusLabel } from "@dhanvi/utils";
import { Breadcrumbs, PageHeader, Button, Callout, ErrorState, Card, CardBody, CardHeader, DataTable, type Column, Dialog, Drawer, Fact, KeyValueRows, FormField, Input, MoneyInput, Textarea, StatusBadge, Badge, Avatar, Icons, PageSkeleton, useToast, useConfirm } from "@dhanvi/ui";
...
function Manage({ scope }: { scope: "admin" | "organizer" }) {
  ...
  const crumbs = [{ label: scope === "organizer" ? "My groups" : "Platform groups", href: `${managePrefix(scope)}/groups` }, { label: data?.group.name ?? "Group", href: `${managePrefix(scope)}/groups/${id}` }, { label: data?.cycle ? `Cycle ${data.cycle.cycleNumber}` : "Cycle", href: `/${scope}/groups/${id}` }, { label: "Contributions" }];
      ...
      <Drawer open={!!history} onClose={() => setHistory(undefined)} title={history ? `${history.memberName} · history` : "History"}>
        {history && (
          <>
            ...
            <Link className="link text-sm" href={`${managePrefix(scope)}/groups/${id}`}>Back to group</Link>
          </>
        )}
      </Drawer>
    </div>
