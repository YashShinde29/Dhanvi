...
import type { Contribution, Member } from "@dhanvi/types";
import { useAsyncData, formatDate, formatMoney, statusLabel } from "@dhanvi/utils";
import { CyclePanel } from "../contributions/cycle-panel";
import { CreatorTypeBadge, GroupTypeBadge, StatusBadge, Breadcrumbs, Card, CardBody, CardHeader, Callout, ErrorState, ProgressBar, Tabs, TabPanel, type TabItem, PageSkeleton, Icons } from "@dhanvi/ui";
import { Guard, managePrefix, managementScope } from "./shared";
...
import { WorkflowStatusCard, StickyActionBar, groupSummary } from "../workflow";
...
function GroupDetail({ scope, applications }: { scope: GroupScope; applications: boolean }) {
  ...
  const [tab, setTab] = useState(applications ? "applications" : requestedTab ?? "overview");
  ...
  const g = group.data;
  const isActiveMember = g?.myMembership?.status === "ACTIVE";
  const cycles = useAsyncData(() => contributionService.cycles(id, manage ? (scope as "admin" | "organizer") : undefined), [id, scope, manage], !!g?.activatedAt && (manage || isActiveMember));
  const mine = useAsyncData(() => contributionService.myGroup(id), [id], !manage && isActiveMember);

  async function refresh() { await Promise.all([group.reload(), manage ? members.reload() : Promise.resolve(), g?.activatedAt ? cycles.reload() : Promise.resolve(), !manage && isActiveMember ? mine.reload() : Promise.resolve()]); }

  if (group.error) return <div className="stack"><Breadcrumbs items={[{ label: manage ? "My groups" : "Browse groups", href: manage ? `${managePrefix(scope)}/groups` : "/groups" }, { label: "Group" }]} /><ErrorState message={group.error} onRetry={group.reload} /></div>;
  ...
  const ownContribution: Contribution | undefined = mine.data?.find((c) => c.cycleId === currentCycle?.id);
  const viewer = scope === "admin" ? "admin" : scope === "organizer" ? "organizer" : "member";
  const progress = groupSummary({ group: g, viewer, scope, currentCycle, ownContribution, members: memberRows, canManage });
  const payDue = !manage && ownContribution && ownContribution.collectionMode === "RAZORPAY" && currentCycle?.status === "COLLECTING_CONTRIBUTIONS" && ownContribution.financialStatus.toUpperCase() !== "SETTLED" ? ownContribution.expectedAmount - ownContribution.financiallySettledAmount : null;
  const showCycles = !!g.activatedAt && (manage || g.myMembership?.status === "ACTIVE");
  const listHref = scope === "organizer" ? "/organizer/groups" : scope === "admin" ? "/groups" : auth.authenticated && g.myMembership ? "/my-groups" : "/groups";
  ...
  const tabs: TabItem[] = manage
    ? [
        ...
        ...(showCycles ? [{ id: "cycles", label: "Cycle progress", alert: currentCycle?.status === "READY_FOR_SELECTION" }] : []),
        { id: "rules", label: "Rules & schedule" },
      ]
    : [
        { id: "overview", label: "Overview" },
        { id: "rules", label: "Rules & schedule" },
        ...(showCycles ? [{ id: "cycles", label: "Cycle progress", alert: !!payDue }] : []),
      ];
  ...
  return (
    <div className={`stack stack--lg${payDue ? " has-sticky-action" : ""}`}>
      <div className="group-hero">
        {scope === "admin" && <Link className="link" href={`/ledger/groups/${id}`}>View group ledger</Link>}
        ...
        <div className="group-hero__top">
          ...
          {manage && !editing && <GroupLifecycleActions group={g} scope={scope} canManage={canManage} termsPending={termsPending} onChanged={refresh} onEdit={() => { setEditing(true); setTab("overview"); }} />}
        </div>
      ...
      {editing ? (
        ...
        <>
          <WorkflowStatusCard summary={progress} viewer={viewer} title="Group progress" stepperLabel="Group lifecycle" />
          {payDue !== null && <StickyActionBar title={`Cycle ${currentCycle?.cycleNumber} contribution due`} amount={payDue} href={`/contributions?groupId=${g.id}`} label="Pay now" />}
          {manage && (
            <div className="grid-4">
              ...
              {currentCycle ? (
                <Link href={`${managePrefix(scope)}/groups/${g.id}/cycles/${currentCycle.id}/contributions`} className={`attention${currentCycle.pendingMemberCount ? " attention--info" : " attention--success"}`}>
                  <span className="attention__count num">{currentCycle.collectionMode === "RAZORPAY" ? currentCycle.financiallySettledMemberCount : currentCycle.fullyRecordedMemberCount}<span className="text-muted" style={{ fontSize: "0.6em" }}> / {currentCycle.expectedMemberCount}</span></span>
                  <span className="attention__text"><span className="attention__title">Cycle {currentCycle.cycleNumber} contributions</span><span className="attention__desc">{formatMoney(currentCycle.collectionMode === "RAZORPAY" ? currentCycle.financiallySettledAmount : currentCycle.recordedContributionAmount)} of {formatMoney(currentCycle.expectedPoolAmount)} {currentCycle.collectionMode === "RAZORPAY" ? "settled" : "recorded"}</span></span>
                </Link>
              ) : (
