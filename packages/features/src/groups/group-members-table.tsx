...
import type { Group, Member } from "@dhanvi/types";
import { Avatar, Badge, StatusBadge, Button, DataTable, type Column, Drawer, DescriptionList, Icons, useToast, useConfirm } from "@dhanvi/ui";
import { friendlyError, formatDate, formatDateTime, statusLabel } from "@dhanvi/utils";
...
export function GroupMembersTable({ group, members, scope, canManage, mode, onChanged, loading }: Props) {
  ...
  async function approve(member: Member) {
    ...
    setBusyId(member.id);
    try { await groupService.action(`${base}/applications/${member.id}/approve`); toast.success("Application approved ✓", `Next: ${member.name} must review and accept the group rules. Their position is reserved.`); await onChanged(); setSelected(null); }
  ...
  async function reject(member: Member) {
    ...
    if (!result.confirmed) return;
    setBusyId(member.id);
    try { await groupService.action(`${base}/applications/${member.id}/reject`, { reason: result.reason }); toast.success("Application rejected", "The applicant can see your reason. The position stays open for others."); await onChanged(); setSelected(null); }
