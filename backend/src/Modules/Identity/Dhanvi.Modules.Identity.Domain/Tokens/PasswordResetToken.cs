...
public sealed class PasswordResetToken : Entity<Guid>
{
    ...
    private PasswordResetToken(Guid id, Guid userId, string hash, DateTimeOffset expiresAt, DateTimeOffset createdAt) : base(id) =>
    ...
    public static PasswordResetToken Create(Guid userId, string hash, DateTimeOffset expiresAt, DateTimeOffset now) => new(Guid.NewGuid(), userId, hash, expiresAt, now);
}
